...
def contests(request, temp):
	...
	if temp == 'a':
		...
		return render(request, 'ContestSpace/contest_page.html', {'content':content, 'button': 'Start Contest', 'link':'startcontest','user_name':request.session.get('username', '123')})
	elif temp == 'b':
		...
		return render(request, 'ContestSpace/contest_page.html', {'content': content, 'user_name':request.session.get('username', '123'), 'button': 'View Result',  'link':'showresult'})
...
def startcontest(request, temp):
	...
	return render(request, 'ContestSpace/startcontest.html', {'teams': teams, 'user_name':request.session['username']})
		...
		if request.POST.get('leave'):
			...
			for u in t:
				...
				if not u.user1 and (not u.user2) and (not u.user3):
					u.delete()
	return render(request, 'ContestSpace/manageteam.html', {'teams': teams, 'pendingreq': pendingreq, 'user_name':request.session.get('username', '123')})
